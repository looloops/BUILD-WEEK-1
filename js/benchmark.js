...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  function startTimer() {
    ...
    timer = setInterval(() => {
      timeLeft--;
      updateTimerDisplay();
      if (timeLeft % (60 / colors.length) === 0) {
        // ogni 20 sec cambia colore(con il push di un nuovo colore calcola il nuono tempo per cambiare il colore)
        changeColor();
      }
      if (timeLeft <= 0) {
        clearInterval(timer);
  ...
  function endQuiz() {
    ...
    document.getElementById("mycircle").addEventListener("mousemove", (event) => {
      ...
      tooltipElement.textContent =
        ...
        "%" +
        " Percentuale incorretta: " +
        percentIncorrect.toFixed(1) +
        "%";
    });
